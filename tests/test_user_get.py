...
# Запрос данных другого пользователя
    def test_get_another_user_detaling(self):
        ...
        response = requests.post("https://playground.learnqa.ru/api/user/", data=data_reg) # регистрация нового пользователя

        data = {
                'email': email,
                'password': password
            }
        response1 = requests.post("https://playground.learnqa.ru/api/user/login", data=data) # авторизация подновым пользователем
